...
**Prior round attempts note (loop memory):** If the user message contains a "## Prior round attempts" section at the top, earlier audit rounds have already run on previous versions of this diff. Each entry shows what was raised that round, the verdict, and the fixer's rationale for what was changed in response. Use this loop memory to:
- **Avoid re-raising** findings the latest fix actually resolves in the current diff. Check the diff before re-raising; do not assume the fix worked, but do not assume it failed either.
- **Re-raise** findings the fix tried to address but failed at — the issue must still be present in the diff under review for you to re-raise it.
- **Avoid proposing approaches the fixer already tried** in an earlier round, unless you can explain concretely why the previous attempt failed and how a new attempt would differ. Repeating a dead-end approach is the failure mode this section exists to prevent.
